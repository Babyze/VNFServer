...
CMD:o(playerid,params[])
{
	...
	for(new x;x<MAX_OSLOTS;x++)
	{
		...
		{	format(string, sizeof(string), ""COL_WHITE"Slot:%d :: "COL_GREEN"Da su dung\n", x);    }
		else format(string, sizeof(string), ""COL_WHITE"Slot:%d\n", x);
		strcat(dialog,string);
   	}
   	ShowPlayerDialog(playerid, DIALOG_ATTACH_INDEX_SELECTION, DIALOG_STYLE_LIST,"Do Choi/Phu Kien: (Chon Cac Slot)", dialog, "Chon", "Dong(X)");
	return 1;
}
CMD:dochoi(playerid,params[])
{
	return cmd_o(playerid,params);
}
CMD:phukien(playerid,params[])
{
	return cmd_o(playerid,params);
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
        case DIALOG_ATTACH_INDEX_SELECTION:
        {
            if(response)
            {
                if(IsPlayerAttachedObjectSlotUsed(playerid, listitem))
                {
                    ...
                    "Do Choi/Phu Kien: (Xoa/Sua)", ""COL_WHITE"Ban muon chinh sua hoac xoa mon phu kien nay?", "Sua(!)", "Xoa(X)");
                }
                else
                {
					ShowPlayerDialog(playerid,DIALOG_ATTACH_OBJECT_SELECTION,DIALOG_STYLE_LIST,"Do Choi/Phu Kien: (Chon Mot Trong Hai)","Path:1 :: "COL_GREY"Danh Sach Cua Server\n"COL_WHITE"Path:2 :: "COL_GREY"Object Tren Mang","Next(>>)","Back(<<)");
     			}
        ...
        case DIALOG_ATTACH_OBJECT_SELECTION:
        {
            ...
            if(response)
            {
            	if(listitem==0) ShowModelSelectionMenuEx(playerid, AttachmentObjectsList, 228+38, "Do Choi/Phu Kien", DIALOG_ATTACH_MODEL_SELECTION, 0.0, 0.0, 0.0, 1.0, 0x00000099, 0x000000EE, 0xACCBF1FF);
            	if(listitem==1) ShowPlayerDialog(playerid,DIALOG_ATTACH_OBJECT2_SELECTION,DIALOG_STYLE_INPUT,"Do Choi/Phu Kien: (Nhap objectid)",""COL_WHITE"Nhap id Object do choi duoi day, Ban co the search Google ''Objects By Category''.","Sua","Back(<<)");
			}
		}
        case DIALOG_ATTACH_OBJECT2_SELECTION:
        {
            if(!response)
            {   ShowPlayerDialog(playerid,DIALOG_ATTACH_OBJECT_SELECTION,DIALOG_STYLE_LIST,"Do Choi/Phu Kien: (Chon)","Path:1 :: "COL_GREY"Danh Sach Cua Server\n"COL_WHITE"Path:2 :: "COL_GREY"Object Tren Mang","Next(>>)","Back(<<)");    }
			if(response)
			{
				if(!strlen(inputtext))return SendClientMessage(playerid,-1,"Error: Ban Khong The De Trong ID Object."),ShowPlayerDialog(playerid,DIALOG_ATTACH_OBJECT2_SELECTION,DIALOG_STYLE_INPUT,"Do Choi/Phu Kien: (Nhap objectid)",""COL_WHITE"Nhap id Object do choi duoi day, Ban co the search Google ''Objects By Category''.","Edit","Back(<<)");
				if(!IsNumeric(inputtext)) return SendClientMessage(playerid,-1,"PLAYER: You can't fill a object name, only object id's allowed."),ShowPlayerDialog(playerid,DIALOG_ATTACH_OBJECT2_SELECTION,DIALOG_STYLE_INPUT,"Do Choi/Phu Kien: (Nhap objectid)",""COL_WHITE"Nhap id Object do choi duoi day, Ban co the search Google ''Objects By Category''.","Sua","Back(<<)");
				new obj;
			    if(!sscanf(inputtext, "i", obj))
				{
				    ...
				    else
				    {
						...
						ShowPlayerDialog(playerid, DIALOG_ATTACH_BONE_SELECTION, DIALOG_STYLE_LIST, \
					 	"{FF0000}Chinh Sua Phu Kien - Chon Vi Tri", dialog, "Chon", "Huy");
				    }
				}
			}
        }
        ...
        case DIALOG_ATTACH_BONE_SELECTION:
        {
            if(response)
            {
                ...
                SendClientMessage(playerid, 0xFFFFFFFF, "Phu Kien/Do Choi: Ban co the giu chuot va xoay chuot de chinh cac vi tri cho mon do phu kien.");
            }
            DeletePVar(playerid, "AttachmentIndexSel");
                                   ...
                                   Float:fScaleX, Float:fScaleY, Float:fScaleZ )
{
    ...
    SendClientMessage(playerid, 0xFFFFFFFF, "Phu Kien/Do Choi: Ban da hoan thanh xong mon phu kien, du lieu da duoc luu.");
...
public OnPlayerModelSelectionEx(playerid, response, extraid, modelid)
{
	...
	{
	    if(!response)
     	{   ShowPlayerDialog(playerid,DIALOG_ATTACH_OBJECT_SELECTION,DIALOG_STYLE_LIST,"Phu Kien/Do Choi: (Chon)","Path:1 :: "COL_GREY"Danh Sach Cua Server\n"COL_WHITE"Path:2 :: "COL_GREY"Object Tren Mang","Next(>>)","Back(<<)");    }
	    if(response)
	    {
