...
{
	Msg(playerid, 0x00AAFFFF, " - Menu Hanh Dong - Menu Animations");
	Msg(playerid, -1, " | - /xemhanhdong {FFFF00}Xem danh sach hanh dong.");
	Msg(playerid, -1, " | - /timkiemhanhdong {FFFF00}Tim kiem hanh dong.");
	Msg(playerid, -1, " | - /idhanhdong {FFFF00}chinh hanh dong theo ID.");
	Msg(playerid, -1, " | - /chinhhanhdong {FFFF00}Chinh sua cho hanh dong.");
	Msg(playerid, -1, " | - /luuhanhdong <comment> {FFFF00}Luu hanh dong '"#ANIM_SAVE_FILE"' va nhan xet.");
	Msg(playerid, -1, " | - /chayhanhdong {FFFF00}Chay lai hanh dong neu hanh dong dung.");
	Msg(playerid, -1, " | - /dunghanhdong {FFFF00}Dung hanh dong.");
	return 1;
}
CMD:xemhanhdong(playerid, params[])
{
    PreloadPlayerAnims(playerid);
	ShowPlayerDialog(playerid, D_ANIM_LIBRARIES, DIALOG_STYLE_LIST, "Chon hanh dong", gLibList, "Open...", "Cancel");
	return 1;
}
CMD:timkiemhanhdong(playerid, params[])
{
	ShowPlayerDialog(playerid, D_ANIM_SEARCH, DIALOG_STYLE_INPUT, "Tim kiem hanh dong", "Nhap hanh dong can kiem", "Open...", "Cancel");
	return 1;
}
CMD:idhanhdong(playerid, params[])
{
	if(gBrowseMode[playerid] == BROWSE_MODE_NONE)
	{
		EnterAnimationBrowser(playerid);
    	Msg(playerid, -1, "Neu chuot khong xuat hien, dung ~k~~VEHICLE_ENTER_EXIT~ de hien chuot.");
    }
    else ExitAnimationBrowser(playerid);
	return 1;
}
CMD:chinhhanhdong(playerid, params[])
{
	FormatAnimSettingsMenu(playerid);
	return 1;
}
CMD:luuhanhdong(playerid, params[])
{
	if(0 < strlen(params) < 32)
	{
		SaveCurrentAnimation(playerid, params);
		Msg(playerid, -1, "Hanh dong da duoc luu!");
	}
	else SendClientMessage(playerid, -1, "Su dung: /luuhanhdong [nhan xet hanh dong 1-32 ki tu]");
	return 1;
}
CMD:chayhanhdong(playerid, params[])
{
    PlayCurrentAnimation(playerid);
	return 1;
}
CMD:dunghanhdong(playerid, params[])
{
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == D_ANIM_LIBRARIES && response)
	{
		...
		ShowPlayerDialog(playerid, D_ANIM_LIST, DIALOG_STYLE_LIST, "Chon hanh dong", gAnimList[listitem], "Play", "Back");
	...
	if(dialogid == D_ANIM_LIST)
	{
		...
		else ShowPlayerDialog(playerid, D_ANIM_LIBRARIES, DIALOG_STYLE_LIST, "Chon hanh dong", gLibList, "Open...", "Cancel");
	}
	if(dialogid == D_ANIM_SEARCH && response)
    {
		...
		else ShowPlayerDialog(playerid, D_ANIM_SEARCH, DIALOG_STYLE_INPUT, "Tim kiem hanh dong", "Nhap hanh dong\n\n{FF0000}Khong tim thay, vui long thu lai.", "Open...", "Cancel");
	}
	if(dialogid == D_ANIM_SEARCH_RESULTS)
	{
	    if(!response)return ShowPlayerDialog(playerid, D_ANIM_SEARCH, DIALOG_STYLE_INPUT, "Tim kiem hanh dong", "Nhap hanh dong", "Open...", "Cancel");
	...
	{
		if(listitem == 0)ShowPlayerDialog(playerid, D_ANIM_SPEED, DIALOG_STYLE_INPUT, "Toc do hanh dong", "Change the speed of the animation below:", "Accept", "Back");
		else if(listitem == 5)ShowPlayerDialog(playerid, D_ANIM_TIME, DIALOG_STYLE_INPUT, "Thoi gian hanh dong", "Thay doi thoi gian:", "Accept", "Back");
		else
		{
	...
	if(dialogid == D_ANIM_IDX && response)
	{
		...
		else ShowPlayerDialog(playerid, D_ANIM_IDX, DIALOG_STYLE_INPUT, "Muc luc hanh dong", "vui long chon muc luc(idx)\ntu 0 and "#MAX_ANIMS":", "Enter", "Cancel");
	}
	return 1;
}
...
public OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
	...
	{
	    ShowPlayerDialog(playerid, D_ANIM_IDX, DIALOG_STYLE_INPUT, "Muc luc hanh dong", "vui long chon muc luc(idx)\ntu 0 and "#MAX_ANIMS":", "Enter", "Cancel");
	}
	if(playertextid == guiCamera)
	{
	    gBrowseMode[playerid] = BROWSE_MODE_CAMERA;
	    PlayerTextDrawSetString(playerid, guiCamera, "~k~~VEHICLE_ENTER_EXIT~ cho mat chuot");
...
FormatAnimSettingsMenu(playerid)
{
	...
	ShowPlayerDialog(playerid, D_ANIM_SETTINGS, DIALOG_STYLE_LIST, "Chinh sua hanh dong", list, "Change", "Exit");
}
